...
#------------------------------------------------------------------------------        
def WriteMap(header_all, map_info, modelfolder=False, ind_number=0, feasible=True, map_postprocess = False):
    ...
    if ind_number != 0 and modelfolder == True:
        ...
        for folder in os.listdir(os.path.join(wrkDir,folder_name)):
            if map_postprocess:
                in_file = open(os.path.join(wrkDir,folder_name,folder,'map_info.asc'), 'w')
            else:
