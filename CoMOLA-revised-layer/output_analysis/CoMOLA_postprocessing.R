...
opt_path <- "C:/Users/Margaux Van Geem/Downloads/CoMOLA-Egypt-paper/output"

# to some post-processing folder
post_path <- "C:/Users/Margaux Van Geem/Downloads/CoMOLA-Egypt-paper/output_analysis"

file_path <- "C:/Users/Margaux Van Geem/Downloads/CoMOLA-Egypt-paper/output_analysis/bestfit.txt"
...
#   cat("File Content:\n")
#   # readLines(file_path, n = 10)  # Change 'n' to read more or fewer lines if needed
# } else {
...
files_matching_pattern <- dir(pattern = "_log")

# Check if any files were found
if (length(files_matching_pattern) > 0) {
  # If files were found, proceed with reading the file
  logfile <- file(files_matching_pattern[1], "r+")  # Assuming you want the first file found
  text <- readLines(logfile)
  close(logfile)
} else {
  # Handle case where no files match the pattern
  print("No files matching the pattern '_log' found.")
}
